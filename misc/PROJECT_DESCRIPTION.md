- use these considerations to make project:
  - project must have a documentation for all section of architectrual design included README.md 
  - Unit-tests must be simple and applicable and cover all needs 
  - for access control design, we need to cover RBAC+ACL if it is possible
  - for UI design, use TailwindCSS 
  - For Authentication and Authorizarion :
      ✔ Manually validate the token (Next.js API Routes)
      ✔ Build a custom login with a React UI
      ✔ Add a refresh token
      ✔ Add two-factor authentication (2FA)
  - add NightMode for UI 
  - all UI design must be responsive
  - for "Maintains historical data for analytics and reporting" use "Versioned Records" method to store all historical data for logging and redoing and also reports must be simple but at the same time must be customizable  
  - for emergency actions must have a simple messaging method to alert related users
